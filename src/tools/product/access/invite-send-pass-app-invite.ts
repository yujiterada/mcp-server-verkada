/**
 * InviteSendPassAppInvite Tool
 *
 * Send an invitation to enable pass for the specified send pass app invite.
 *
 * @category product/access
 * @operationId postSendPassAppInviteViewV1
 * @method POST
 * @path /access/v1/access_users/user/pass/invite
...
/**
 * Input parameters for inviteSendPassAppInvite
 */
const InviteSendPassAppInviteInputSchema = z.object({
  /** Path parameters */
  query: z.object({
...
type InviteSendPassAppInviteInput = z.infer<typeof InviteSendPassAppInviteInputSchema>;
...
/**
 * Output schema for inviteSendPassAppInvite
 * OK
 */
const InviteSendPassAppInviteOutputSchema = z.object({
...
type InviteSendPassAppInviteOutput = z.infer<typeof InviteSendPassAppInviteOutputSchema>;
 ...
 * @returns OK
 */
export async function inviteSendPassAppInvite(
  input: InviteSendPassAppInviteInput
): Promise<APIResponse<InviteSendPassAppInviteOutput>> {
  // Validate input
  const validated = InviteSendPassAppInviteInputSchema.parse(input);

  // Build path with parameters
  const path = '/access/v1/access_users/user/pass/invite';
  ...
  // Make API request
  const response = await callVerkadaAPI<InviteSendPassAppInviteOutput>({
    method: 'POST',
    path: fullPath,
  });
  ...
  if (response.success && response.data) {
    try {
      response.data = InviteSendPassAppInviteOutputSchema.parse(response.data);
    } catch (error) {
 ...
 */
export const inviteSendPassAppInviteMetadata = {
  name: 'invite_send_pass_app_invite',
  description: `Send an invitation to enable pass for the specified send pass app invite.`,
  inputSchema: InviteSendPassAppInviteInputSchema,
  outputSchema: InviteSendPassAppInviteOutputSchema,
  category: 'product/access',
  operationId: 'postSendPassAppInviteViewV1',
