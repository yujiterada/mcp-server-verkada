/**
 * UpdateAccessUserPin Tool
 *
 * Update an existing access user pin. Only the provided fields will be changed.
 *
...
/**
 * Input parameters for updateAccessUserPin
 */
const UpdateAccessUserPinInputSchema = z.object({
  /** Path parameters */
  query: z.object({
  ...
  /** Body parameters */
  body: z.object({
    /** The access entry code used for keypad readers alone or in 2FA mode. Formatted as a string of digits between 4 and 16 in length. Must be unique within the organization. (required) */
    entry_code: z.string(),
  }),
});

type UpdateAccessUserPinInput = z.infer<typeof UpdateAccessUserPinInputSchema>;
...
/**
 * Output schema for updateAccessUserPin
 * OK
 */
const UpdateAccessUserPinOutputSchema = z.object({
...
type UpdateAccessUserPinOutput = z.infer<typeof UpdateAccessUserPinOutputSchema>;
 ...
 * @param input.query.override - The override parameter
 * @param input.body.entry_code - The access entry code used for keypad readers alone or in 2FA mode. Formatted as a string of digits between 4 and 16 in length. Must be unique within the organization.
 * @returns OK
 */
export async function updateAccessUserPin(
  input: UpdateAccessUserPinInput
): Promise<APIResponse<UpdateAccessUserPinOutput>> {
  // Validate input
  const validated = UpdateAccessUserPinInputSchema.parse(input);
  ...
  if (validated.query.override !== undefined) {
    queryParams.set('override', String(validated.query.override));
  }
  const queryString = queryParams.toString();
  const fullPath = queryString ? `${path}?${queryString}` : path;

  // Make API request
  const response = await callVerkadaAPI<UpdateAccessUserPinOutput>({
    method: 'PUT',
    path: fullPath,
    body: {
      entry_code: validated.body.entry_code,
    },
  });
  ...
  if (response.success && response.data) {
    try {
      response.data = UpdateAccessUserPinOutputSchema.parse(response.data);
    } catch (error) {
 ...
 */
export const updateAccessUserPinMetadata = {
  name: 'update_access_user_pin',
  description: `Update an existing access user pin. Only the provided fields will be changed.`,
  inputSchema: UpdateAccessUserPinInputSchema,
  outputSchema: UpdateAccessUserPinOutputSchema,
  category: 'product/access',
  operationId: 'putAccessUserPinViewV1',
  method: 'PUT' as const,
  path: '/access/v1/access_users/user/entry_code',
