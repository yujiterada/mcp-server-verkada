/**
 * UpdateAccessEndDate Tool
 *
 * Update an existing access end date. Only the provided fields will be changed.
 *
 * @category product/access
 * @operationId putAccessEndDateViewV1
 * @method PUT
 * @path /access/v1/access_users/user/end_date
...
/**
 * Input parameters for updateAccessEndDate
 */
const UpdateAccessEndDateInputSchema = z.object({
  /** Path parameters */
  query: z.object({
...
type UpdateAccessEndDateInput = z.infer<typeof UpdateAccessEndDateInputSchema>;
...
/**
 * Output schema for updateAccessEndDate
 * OK
 */
const UpdateAccessEndDateOutputSchema = z.object({
...
type UpdateAccessEndDateOutput = z.infer<typeof UpdateAccessEndDateOutputSchema>;
 ...
 * @param input.body.end_date - The start time value of the time range controlling when a users credentials are active. Formatted as a Unix timestamp in seconds.
 * @returns OK
 */
export async function updateAccessEndDate(
  input: UpdateAccessEndDateInput
): Promise<APIResponse<UpdateAccessEndDateOutput>> {
  // Validate input
  const validated = UpdateAccessEndDateInputSchema.parse(input);

  // Build path with parameters
  const path = '/access/v1/access_users/user/end_date';
  ...
  // Make API request
  const response = await callVerkadaAPI<UpdateAccessEndDateOutput>({
    method: 'PUT',
    path: fullPath,
    body: {
      end_date: validated.body.end_date,
    },
  });
  ...
  if (response.success && response.data) {
    try {
      response.data = UpdateAccessEndDateOutputSchema.parse(response.data);
    } catch (error) {
 ...
 */
export const updateAccessEndDateMetadata = {
  name: 'update_access_end_date',
  description: `Update an existing access end date. Only the provided fields will be changed.`,
  inputSchema: UpdateAccessEndDateInputSchema,
  outputSchema: UpdateAccessEndDateOutputSchema,
  category: 'product/access',
  operationId: 'putAccessEndDateViewV1',
  method: 'PUT' as const,
  path: '/access/v1/access_users/user/end_date',
