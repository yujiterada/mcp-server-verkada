/**
 * UpdateAccessStartDate Tool
 *
 * Update an existing access start date. Only the provided fields will be changed.
 *
 * @category product/access
 * @operationId putAccessStartDateViewV1
 * @method PUT
 * @path /access/v1/access_users/user/start_date
...
/**
 * Input parameters for updateAccessStartDate
 */
const UpdateAccessStartDateInputSchema = z.object({
  /** Path parameters */
  query: z.object({
  ...
  /** Body parameters */
  body: z.object({
    /** The start time value of the time range controlling when a users credentials are active. Formatted as a Unix timestamp in seconds. (required) */
    start_date: z.string(),
  }),
});

type UpdateAccessStartDateInput = z.infer<typeof UpdateAccessStartDateInputSchema>;
...
/**
 * Output schema for updateAccessStartDate
 * OK
 */
const UpdateAccessStartDateOutputSchema = z.object({
...
type UpdateAccessStartDateOutput = z.infer<typeof UpdateAccessStartDateOutputSchema>;
 ...
 * @param input.body.start_date - The start time value of the time range controlling when a users credentials are active. Formatted as a Unix timestamp in seconds.
 * @returns OK
 */
export async function updateAccessStartDate(
  input: UpdateAccessStartDateInput
): Promise<APIResponse<UpdateAccessStartDateOutput>> {
  // Validate input
  const validated = UpdateAccessStartDateInputSchema.parse(input);

  // Build path with parameters
  const path = '/access/v1/access_users/user/start_date';
  ...
  // Make API request
  const response = await callVerkadaAPI<UpdateAccessStartDateOutput>({
    method: 'PUT',
    path: fullPath,
    body: {
      start_date: validated.body.start_date,
    },
  });
  ...
  if (response.success && response.data) {
    try {
      response.data = UpdateAccessStartDateOutputSchema.parse(response.data);
    } catch (error) {
 ...
 */
export const updateAccessStartDateMetadata = {
  name: 'update_access_start_date',
  description: `Update an existing access start date. Only the provided fields will be changed.`,
  inputSchema: UpdateAccessStartDateInputSchema,
  outputSchema: UpdateAccessStartDateOutputSchema,
  category: 'product/access',
  operationId: 'putAccessStartDateViewV1',
  method: 'PUT' as const,
  path: '/access/v1/access_users/user/start_date',
