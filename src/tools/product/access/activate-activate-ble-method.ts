/**
 * ActivateActivateBleMethod Tool
 *
 * Enable BLE unlock for the specified activate ble method.
 *
 * @category product/access
 * @operationId putActivateBLEMethodViewV1
 * @method PUT
 * @path /access/v1/access_users/user/ble/activate
...
/**
 * Input parameters for activateActivateBleMethod
 */
const ActivateActivateBleMethodInputSchema = z.object({
  /** Path parameters */
  query: z.object({
...
type ActivateActivateBleMethodInput = z.infer<typeof ActivateActivateBleMethodInputSchema>;
...
/**
 * Output schema for activateActivateBleMethod
 * OK
 */
const ActivateActivateBleMethodOutputSchema = z.object({
...
type ActivateActivateBleMethodOutput = z.infer<typeof ActivateActivateBleMethodOutputSchema>;
 ...
 * @returns OK
 */
export async function activateActivateBleMethod(
  input: ActivateActivateBleMethodInput
): Promise<APIResponse<ActivateActivateBleMethodOutput>> {
  // Validate input
  const validated = ActivateActivateBleMethodInputSchema.parse(input);

  // Build path with parameters
  const path = '/access/v1/access_users/user/ble/activate';
  ...
  // Make API request
  const response = await callVerkadaAPI<ActivateActivateBleMethodOutput>({
    method: 'PUT',
    path: fullPath,
  });
  ...
  if (response.success && response.data) {
    try {
      response.data = ActivateActivateBleMethodOutputSchema.parse(response.data);
    } catch (error) {
 ...
 */
export const activateActivateBleMethodMetadata = {
  name: 'activate_activate_ble_method',
  description: `Enable BLE unlock for the specified activate ble method.`,
  inputSchema: ActivateActivateBleMethodInputSchema,
  outputSchema: ActivateActivateBleMethodOutputSchema,
  category: 'product/access',
  operationId: 'putActivateBLEMethodViewV1',
  method: 'PUT' as const,
  path: '/access/v1/access_users/user/ble/activate',
