/**
 * CreateAccessDoorExceptions Tool
 *
 * Create a new access door exceptions. Provide the required fields in the request body.
 *
 * @category product/access
 * @operationId postAccessDoorExceptionsViewV1
 * @method POST
 * @path /access/v1/door/exception_calendar/{calendar_id}/exception
...
/**
 * Input parameters for createAccessDoorExceptions
 */
const CreateAccessDoorExceptionsInputSchema = z.object({
  /** Path parameters */
  path: z.object({
...
&lt;code&gt;first_person_in&lt;/code&gt; must also be set to &lt;code&gt;TRUE&lt;/code&gt; if this value is provided. */
    ...
    recurrence_rule: z.object({ by_day: z.array(z.enum(['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'])).optional(), by_month: z.number().int().optional(), by_month_day: z.number().int().optional(), by_set_pos: z.number().int().optional(), count: z.number().int().optional(), excluded_dates: z.array(z.string()).optional(), frequency: z.enum(['DAILY', 'WEEKLY', 'MONTHLY', 'YEARLY']), interval: z.number().int(), until: z.string().optional() }).optional(),
    /** Start time of the Door Exception formatted as &lt;code&gt;hh:mm&lt;/code&gt; (according to ISO 8601). (required) */
    start_time: z.string().time(),
  }),
});

type CreateAccessDoorExceptionsInput = z.infer<typeof CreateAccessDoorExceptionsInputSchema>;
...
/**
 * Output schema for createAccessDoorExceptions
 * OK
 */
const CreateAccessDoorExceptionsOutputSchema = z.object({
  /** Bool value specifying if Exception is an All Day Default. If &lt;code&gt;TRUE&lt;/code&gt;, then:
- &lt;code&gt;door_status&lt;/code&gt; must be set to &lt;code&gt;access_controlled&lt;/code&gt;
- &lt;code&gt;start_time&lt;/code&gt; and &lt;code&gt;end_time&lt;/code&gt; will automatically be set to &lt;code&gt;00:00:00&lt;/code&gt; and &lt;code&gt;23:59:59&lt;/code&gt;, respectively. Different values should not be provided.
- &lt;code&gt;first_person_in&lt;/code&gt; and &lt;code&gt;double_badge&lt;/code&gt; must be &lt;code&gt;FALSE&lt;/code&gt; */
 ...
 * @returns OK
 */
export async function createAccessDoorExceptions(
  input: CreateAccessDoorExceptionsInput
): Promise<APIResponse<CreateAccessDoorExceptionsOutput>> {
  // Validate input
  const validated = CreateAccessDoorExceptionsInputSchema.parse(input);

  // Build path with parameters
  let path = '/access/v1/door/exception_calendar/{calendar_id}/exception';
  path = path.replace('{calendar_id}', encodeURIComponent(String(validated.path.calendar_id)));

  const fullPath = path;

  // Make API request
  const response = await callVerkadaAPI<CreateAccessDoorExceptionsOutput>({
    method: 'POST',
    path: fullPath,
    body: {
 ...
 */
export const createAccessDoorExceptionsMetadata = {
  name: 'create_access_door_exceptions',
  description: `Create a new access door exceptions. Provide the required fields in the request body.`,
  inputSchema: CreateAccessDoorExceptionsInputSchema,
  outputSchema: CreateAccessDoorExceptionsOutputSchema,
  category: 'product/access',
  operationId: 'postAccessDoorExceptionsViewV1',
