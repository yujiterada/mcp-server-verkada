...
const ListAlarmsDevicesOutputSchema = z.object({
  ...
  devices: z.array(z.object({ device_id: z.string(), device_type: z.enum(['door_contact_sensor', 'glass_break_sensor', 'motion_sensor', 'panic_button', 'water_sensor', 'wired_door_contact_sensor', 'wired_generic_sensor', 'wired_glass_break_sensor', 'wired_motion_sensor', 'wired_panic_button', 'wired_smoke_sensor', 'wired_water_sensor', 'wireless_relay']), site_id: z.string() })),
});
...
export async function listAlarmsDevices(
  ...
  if (response.success && response.data) {
    try {
      response.data = ListAlarmsDevicesOutputSchema.parse(response.data);
    } catch (error) {
