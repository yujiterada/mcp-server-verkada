/**
 * ActivateActivateRemoteUnlock Tool
 *
 * Enable remote unlock for the specified activate remote unlock.
 *
 * @category product/access
 * @operationId putActivateRemoteUnlockViewV1
 * @method PUT
 * @path /access/v1/access_users/user/remote_unlock/activate
...
/**
 * Input parameters for activateActivateRemoteUnlock
 */
const ActivateActivateRemoteUnlockInputSchema = z.object({
  /** Path parameters */
  query: z.object({
...
type ActivateActivateRemoteUnlockInput = z.infer<typeof ActivateActivateRemoteUnlockInputSchema>;
...
/**
 * Output schema for activateActivateRemoteUnlock
 * OK
 */
...
type ActivateActivateRemoteUnlockOutput = z.infer<typeof ActivateActivateRemoteUnlockOutputSchema>;
 ...
 * @returns OK
 */
export async function activateActivateRemoteUnlock(
  input: ActivateActivateRemoteUnlockInput
): Promise<APIResponse<ActivateActivateRemoteUnlockOutput>> {
  // Validate input
  const validated = ActivateActivateRemoteUnlockInputSchema.parse(input);

  // Build path with parameters
  const path = '/access/v1/access_users/user/remote_unlock/activate';
  ...
  // Make API request
  const response = await callVerkadaAPI<ActivateActivateRemoteUnlockOutput>({
    method: 'PUT',
    path: fullPath,
  });
  ...
  if (response.success && response.data) {
    try {
      response.data = ActivateActivateRemoteUnlockOutputSchema.parse(response.data);
    } catch (error) {
 ...
 */
export const activateActivateRemoteUnlockMetadata = {
  name: 'activate_activate_remote_unlock',
  description: `Enable remote unlock for the specified activate remote unlock.`,
  inputSchema: ActivateActivateRemoteUnlockInputSchema,
  outputSchema: ActivateActivateRemoteUnlockOutputSchema,
  category: 'product/access',
  operationId: 'putActivateRemoteUnlockViewV1',
  method: 'PUT' as const,
  path: '/access/v1/access_users/user/remote_unlock/activate',
